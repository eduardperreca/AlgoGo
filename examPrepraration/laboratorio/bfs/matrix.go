...
func main() {
	...
	for len(queue) > 0 {
		...
		// 5. Dequeue the first node
		node := queue[0]
		queue = queue[1:]
		fmt.Println(node)
		// 6. Check if it is the goal
		if matrix[node.Row][node.Column] == 2 {
			println("END!, we spent", len(visited), "steps")
			return
		}

		// 7. Add the neighbors to the queue
		if node.Row > 0 {
			if _, ok := visited[Node{node.Row - 1, node.Column}]; !ok && matrix[node.Row-1][node.Column] == 0 || matri[node.Row-1][node.Column] == 2 {
